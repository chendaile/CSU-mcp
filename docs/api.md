...
```shell
git clone https://github.com/your-org/campusapp.git
cd campusapp
...
### System Structure
项目仍采用 beego 的 MVC 模式，但现在以功能维度统一在仓库根目录下管理。核心目录说明如下：

```
├── cmd
│   ├── api-server         # beego API 入口
│   └── mcp-proxy          # MCP HTTP 入口
├── configs
│   └── api/conf/app.conf  # API 服务配置
├── internal
│   ├── api
│   │   ├── controllers
│   │   ├── middleware
...
│   │   └── utils
│   └── mcp
│       ├── campus         # csugo API client
│       ├── config         # MCP server 配置解析
│       └── tools          # MCP 工具实现
├── tests
│   └── api/integration    # beego 端到端测试
├── third_party
...
│   ├── static             # 静态资源
│   └── views              # 模板
└── Dockerfile / docker-entrypoint.sh 等部署文件
```
